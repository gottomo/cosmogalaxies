...
i=0
for model in models:
	datafile=open(os.path.join(cur_path, "data//RelDif_constPhiAlpha_zref") + str(z_ref) + "_m" + str(magnitude_min) + "_" + modelname[i] + ".txt", 'w')
...
for model in models:
	datafile=open(os.path.join(cur_path, "data//RelDif_varAlpha_zref") + str(z_ref) + "_m" + str(magnitude_min) + "_" + modelname[i] + ".txt", 'w')
	data = np.stack((z, model), axis=1)
	np.savetxt(datafile, data)
	i=i+1
	datafile.close()
...
i=0
for model in models:
	datafile=open(os.path.join(cur_path, "data//RelDif_varPhi_zref") + str(z_ref) + "_m" + str(magnitude_min) + "_" + modelname[i] + ".txt", 'w')
	data = np.stack((z, model), axis=1)
	np.savetxt(datafile, data)
	i=i+1
	datafile.close()
