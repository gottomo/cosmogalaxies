...
def Single_schechter_mass(mass, break_mass, phi, alpha):
	# Returns Schechter function as a function of mass
	# mass shall be given in the unit of dex (i.e. mass = log10(mass_in_solar_mass) )
	# Will be in unit of dex^-1 Mpc^-3
	mass_diff = mass - break_mass
	return np.log(10) * np.exp(- np.power(10, mass_diff)) * (phi * np.power(10, alpha * mass_diff)) * np.power(10, mass_diff)
...
def SchechterPDF_norm(Start, End, break_mass, phi1, phi2, alpha1, alpha2):
...
Single_schechter_mass_vec = np.vectorize(Single_schechter_mass)
...
BinFinesse = 20 # bins per dex mass on the x axis
...
plt.hist(Galaxies, bins = NumberOfBins, alpha = 0.7)
...
# Fitting to see if we get back our expected Schechter function from this sampling:

# Set of bar heights to fit to
FitToSchechter = np.histogram(Galaxies, bins = NumberOfBins, range = (8,12))
# THE BOXES AREN'T NORMALIZED!!!

# Now we calculate the midpoint of each bin; we'll approximate by saying that 
# this is the x value of our datapoint.
mid = np.array([(a + b) /2 for a,b in zip(FitToSchechter[1][:-1], FitToSchechter[1][1:])])

# The counting error on the number of galaxies in each bin will be the square root of
# the count
CountErr = np.sqrt(FitToSchechter[0])

norm = SchechterPDF_norm(8, 12, 11.32, 10**(-3.2), 0, -1.41, 0)
# Check the norm, and check that both the normalized schechter and the histogram
# sum to the number of galaxies
print(norm)
print("Number of galaxies from the histogram:")
print(np.sum(FitToSchechter[0]))
print("Number of galaxies from integrating Schechter re-normalized:")
print(SchechterPDF_norm(8, 12, 11.32, (GalaxyNumber/norm)*10**(-3.2), 0, -1.41, 0))

print("\n \n")

# This is our Schechter funciton normalized so that it will fit on top of our histograms.
# To do this we take into account three things; the norm, which let's us normalize schechter
# so that it's the distribuion of one galaxy, ie probability this galaxy is any one mass,
# then the number of galaxies; we multiply the one galaxy probability distribution by
# our total number of galaxies.
# Finally, we consider the width of our bins; whereas the units of schechter are per
# dex of mass, the units of our histgram bars are per dM a small width of a bar:

# The analytical Schechtr funciton, re-normalized to fit the data, is
DoesTheFitFit = schechter_mass_vec(MassArray, 11.32, (GalaxyNumber/(norm*BinFinesse))*10**(-3.2), 0, -1.41, 0)


#Now let's do some actual fitting

# Fit to the double schechter function, leaving all parameters free
# Too many parameters for scipy to converge properly, gives very different parameters every
# run, and probablt is the cause of 90% of failures to converge
popt, pcov = curve_fit(schechter_mass_vec, mid, FitToSchechter[0], p0 = [11.32, (GalaxyNumber/(norm*BinFinesse))*10**(-3.2), 0, -1.41, 0])
print("The fit parameters that give the best fit using the double schechter function are:")
print(popt)

# Provides a much better fit.
poptSingle, pcovSingle = curve_fit(Single_schechter_mass_vec, mid, FitToSchechter[0], p0 = [11.32, (GalaxyNumber/(norm*BinFinesse))*10**(-3.2), -1.41])
print("Best fit parameters for the single Schchter function are:")
print(poptSingle)

print("The true parameters of the re-normalized version of the schechter function this data is samples from, are:")
print(11.32, (GalaxyNumber/(norm*BinFinesse))*10**(-3.2), -1.41)


# Using these parameters, let's plot what the fit looks like
TisTheFit = schechter_mass_vec(MassArray, popt[0], popt[1], popt[2], popt[3], popt[4])
TisTheFitSingle = Single_schechter_mass_vec(MassArray, poptSingle[0], poptSingle[1], poptSingle[2])

plt.scatter(mid, FitToSchechter[0], marker = "x", label = "Data to fit to") #scatter plot of the data we used to fit to
plt.plot(MassArray, TisTheFit, color= "red", label = "Double schechter fit")
plt.plot(MassArray, TisTheFitSingle, color= "green", label = "Single schechter fit")

#plt.hist(Galaxies, bins = NumberOfBins, alpha = 0.3)
...
# Fits that won;t converge
#poptStrict, pcovStrict = curve_fit(schechter_mass_vec, mid, FitToSchechter[0], p0 = [11.32, (GalaxyNumber/(norm*BinFinesse))*10**(-3.2), 0, -1.41, 0], bounds = ([0, -np.inf, 0, -np.inf, 0], [100, np.inf, 0, np.inf, 0]))
#poptErr, pcovErr = curve_fit(schechter_mass_vec, mid, FitToSchechter[0], p0 = [11.32, (GalaxyNumber/(norm*BinFinesse))*10**(-3.2), 0, -1.41, 0], sigma = CountErr)
#TisTheFitStrict = schechter_mass_vec(MassArray, poptStrict[0], poptStrict[1], poptStrict[2], poptStrict[3], poptStrict[4])
#TisTheFitErr = schechter_mass_vec(MassArray, poptErr[0], poptErr[1], poptErr[2], poptErr[3], poptErr[4])

#plt.plot(MassArray, TisTheFitStrict, color= "orange")
#plt.plot(MassArray, TisTheFitErr, color= "purple")
...
plt.plot(MassArray, TisTheFit, color= "red", label = "Double Schechter fit")
plt.plot(MassArray, TisTheFitSingle, color= "green", label = "Single Schechter fit")
plt.plot(MassArray, DoesTheFitFit, label = "Original scaled", color = "Blue", linestyle = ":")
plt.yscale("log")
plt.legend()
plt.show()


#fit = fitChisq.ls_fit(mid, FitToSchechter[0], np.sqrt(FitToSchechter[0]), schechter_mass)


plt.hist(Galaxies, bins = NumberOfBins, alpha = 0.7)

plt.plot(MassArray, DoesTheFitFit, label = "Original scaled")
#plt.plot(MassArray, TisTheFit, label = "Fitted")
